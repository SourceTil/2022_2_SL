...
class MockIdleManager : public IdleManager {
 public:
  MOCK_METHOD(milliseconds, takeSleepTime, (), ());
  MOCK_METHOD(void, process_idling, (), ());
};
...
TEST(Config, get_value) {
  ...
  EXPECT_CALL(mockConfig, get_value("service", "name")).Times(1);
  mockConfig.get_value("service", "name");
}
